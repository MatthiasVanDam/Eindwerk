    ...
    if(in_array($fileActualExt,$allowed)){
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
                ...
                if(empty($_POST['realisatieId'])){
                    header("location: ../fotoToevoegen.php?realiastieId=empty");
                    exit();
                }
                else{
                    ...
                    if(!mysqli_stmt_prepare($stmt,$sql)){
                        ...
                        $sql= "INSERT INTO photos (realisatie_id,image,type,size) VALUES (?,?,?,?)";
                        if(!mysqli_stmt_prepare($stmt,$sql)){
                            echo "Sql statement failed";
                        }
                        else{
                            mysqli_stmt_bind_param($stmt, "ssss", $realisatieId, $imageFullName, $fileType, $fileSize);
                            mysqli_stmt_execute($stmt);
